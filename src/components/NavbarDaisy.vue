<template>
  <div class="navbar bg-base-100">
    <div
      class="flex-1 text-4xl font-bold bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500 font-montserrat text-transparent bg-clip-text"
    >
      <a href="/" class="btn btn-ghost text-xl font-monserrat">AnimeFlix</a>
    </div>

    <div class="flex-none gap-2">
      <!-- Ini search untuk mode desktop -->
      <div
        class="hidden sm:flex items-center border border-gray-400 rounded-lg px-4 py-2 w-96 mr-2"
      >
        <svg
        ...
        >
          <path
            ...
            stroke-width="2"
            d="M21 21l-4.35-4.35m0 0a7.5 7.5 0 111.5-1.5m-1.5 1.5l4.35 4.35"
          />
        </svg>
        <input
          type="text"
      ...
      <div class="dropdown dropdown-end">
        <label tabindex="0" class="btn btn-ghost btn-circle avatar" @click="isOpen = !isOpen">
          <div class="w-10 rounded-full">
            <img v-if="user?.profile?.image" alt="User profile" :src="user.profile.image" />
            <img
              v-else
              alt="Default profile"
